...
matchbox-window-manager -use_titlebar no -use_lowlight no &  # minimal WM
...
minimalist WM designed for small/embedded screens, and its whole design is
\emph{one application window at a time, sized to fill the display}. That is
what makes Chromium's kiosk mode and Tux Paint actually fill the screen: on bare
X, with no WM to grant the fullscreen request, Chromium sits in a 1313$\times$1056
window in the corner. The only thing left to turn off is the title bar. Matchbox
makes one exception: a window that declares itself a \emph{dock}
(\code{\_NET\_WM\_WINDOW\_TYPE\_DOCK}) is pinned to a screen edge and the app
is shrunk to fit around it --- the hook for anything that must stay on screen
\emph{alongside} an app, such as a time-limit countdown. A trap worth recording:
both options take a yes/no value, and a bare \code{-use\_lowlight} makes matchbox
print its usage and exit, silently leaving the session with no window manager.
...
free. You can legally run X with \emph{no} window manager at all, and kidbox
did so by accident for a day: windows then appear wherever the app asks, can't
be moved, and nobody grants a fullscreen request.
\end{deeper}
