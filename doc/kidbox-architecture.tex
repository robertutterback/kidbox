...
matchbox-window-manager -use_titlebar no -use_lowlight & # minimal WM
...
not an option kidbox switches on, it is what matchbox does. The only thing left
to turn off is the title bar, so the app fills the screen with no decorations to
poke at. Matchbox makes one exception: a window that declares itself a
\emph{dock} (\code{\_NET\_WM\_WINDOW\_TYPE\_DOCK}) is pinned to a screen
edge and stays visible, and the app is shrunk to fit around it. That is the hook
for anything that must be on screen \emph{alongside} an app, such as a
