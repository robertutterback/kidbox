...
\item \textbf{No window manager.} A window manager decides where windows go
and draws their frames. kidbox does not run one: Chromium's kiosk mode, Tux
Paint's SDL fullscreen and \code{xterm -maximized} each size themselves to the
whole screen, so there is nothing to manage and no decorations to poke at. (For
months this script started \texttt{matchbox-window-manager} with a made-up
\code{-use\_fullscreen} flag. Matchbox exits on an unknown option, so it never
ran. When the flag was removed and matchbox really started, it ignored
Chromium's fullscreen request and left it in a small window --- bare X was the
configuration that worked all along.) Two things follow from having no WM:
keyboard focus follows the mouse pointer, and stacking is simply map order, so
anything that must stay visible over an app has to raise itself.
...
handles moving, resizing, and focus. That separation is why the same
application runs unchanged under \texttt{i3}, \texttt{openbox}, or a full
GNOME shell. kidbox exploits the separation to the limit by running \emph{no}
window manager at all, which X permits: windows appear wherever the app asks,
can't be moved, and a ``fullscreen'' app is just a window the size of the
screen. One app that sizes itself to the screen is a kiosk for free.
\end{deeper}
...
\texttt{whiptail}, \texttt{xbindkeys},
...
\texttt{xinit} over a display manager; no window manager over a desktop
