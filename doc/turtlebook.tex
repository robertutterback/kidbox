...
\draftnote{Useful words about words: \texttt{po "square} shows the lesson
  ...
  Words are forgotten when Logo closes. \texttt{save "mine.lg} tells Logo
  to remember them all under the name \texttt{mine.lg}, and next time
  \texttt{load "mine.lg} brings every one of them back.}
% Author note: the menu starts Logo inside kidbox/logo, so the saved file
% always ends up where load looks. No need to explain that to the reader.
...
\draftnote{The strangest and best trick. \texttt{spiral} draws one side,
  ...
  (\key{Ctrl}+\key{C} does nothing while a program runs). If all else
  fails, \key{Ctrl}+\key{Alt}+\key{Backspace} leaves Logo for the menu,
  as the kidbook says.}
% Author note: the Logo menu lists Alt+S next to Stop, but it does not
% respond on the Pi (checked 2026-09-04), so the book only shows the mouse.
