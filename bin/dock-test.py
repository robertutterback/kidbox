...
Not installed by install.sh. To try it, with a website open on the Pi:

  1. Ctrl+Alt+F2, log in as the kid user (shares ~/.Xauthority with :1).
  2. sudo apt-get install python3-tk        (once; not in APT_PACKAGES yet)
  3. DISPLAY=:1 python3 ~/kidbox/bin/dock-test.py 120
  4. Ctrl+Alt+F1 (or F7) back to the X screen.
